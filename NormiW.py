...
class NormiAdr():
    ...
    def __init__(self,inp:str,Tp='W',i=0,) -> None:
        ...
        ~inp.find(' Корпус ')and(inp:=inp.replace(' Корпус ','/',1)) # по апрелю (у мэка )
        ~inp.find('корп. ')and(inp:=inp.replace('корп. ','корп.',1)) # по апрелю (у мэка )
        ~inp.find(', кв.Гараж ')and(inp:=inp.replace(', кв.Гараж ','Гараж, кв.',1)) #  гаражи как вид литеры :)
        
        isW and ~inp.find('стр.')and(inp:=inp.replace('стр.','корп.',1)) # СалЩед д.3 стр.1 и 2 и Советская д123А стр НО г.Магнитогорск, ул.Некрасова, д.28 корп.стр.1, кв.0
