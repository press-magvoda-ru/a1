...
def DistribResultByChanksForFollowZiping(fld='.',sz=600_000_000):
    ...
    def teeprint(arg):
        ...
        print(fn)
        print(*arg,sep='\n')
        nm=join(fld,fn+f'_lst({str(datetime.now()).replace(':','').split('.')[0]}).txt')
        print(*arg,sep='\n',file=open(nm,'wt'),end='')
        return nm
    l=[v for v in open(join(fld,fscv)).read().split(',\n') if v]
    o=[]
    for v in l:
        fn,pars,Whom=v.split(',')
        o.append([Whom,int(pars)>0,fsz:=getsize(join(fld,fn:=fn+'.pdf')),fn])
    o.sort(reverse=1)
    teeprint(o)
    sq=[]
    mfo,sum,num,cur='',0,1,''
